...
constexpr const char* TMP_PATH = "/tmp/PVN.socket";
constexpr const char* VAR_PATH = "/var/run/PVN/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("PVN")) {
    logger.debug() << "/var/run/PVN seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("PVN")) {
    logger.warning() << "Failed to create /var/run/PVN";
    return TMP_PATH;
  }

  if (chmod("/var/run/PVN", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/PVN";
    return TMP_PATH;
  }
